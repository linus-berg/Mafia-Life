private ["_client", "_gear", "_uniform", "_vest", "_backpack", "_goggles", "_headgear", "_assignedItems", "_primary", "_primary_i", "_handgun", "_handgun_i", "_container", "_uniform_w", "_uniform_m", "_uniform_i", "_backpack_w", "_backpack_m", "_backpack_i", "_vest_w", "_vest_m", "_vest_i"];
_client = [_this, 0, objNull, [objNull]] call BIS_fnc_param;
_gear = [_this, 1, [], [[]]] call BIS_fnc_param;
if((count _gear) <= 0)exitWith{SystemChat "ERROR: No gear list provided."; false};
...
if(_goggles != "")then{_client forceAddUniform _uniform;};
if(_headgear != "")then{_client addHeadgear _headgear;};
...
if(_uniform != "")then{
  ...
  {
    _container addMagazineCargo [_x, (_uniform_m select 1) select _forEachIndex];
...
if(_backpack != "")then{
  ...
  {
    _container addWeaponCargo [_x, (_backpack_w select 1) select _forEachIndex];
  }forEach (_backpack_w select 0);
  {
    _container addMagazineCargo [_x, (_backpack_m select 1) select _forEachIndex];
