...
_npc removeAction (missionNameSpace getVariable 
                   [(format["%1_Quest_Action_HandIn", (_quest select 0)]), 0]);
...
[(_quest select 3), 
 (_quest select 5), (_quest select 6), _caller] call ML_fnc_Quests_Rewards;
...
if ( [(_quest select 0), _npc] in _quests ) then {
  ...
  if !( _completed in _quests_completed ) then {
    ...
    ["ML_Info", ["Quest Completed!", "\ML_Images\Actions\Quest_Icon.paa", 
                 format["<t color='#01DF01'>You completed: '%1'!</t>", 
                        (_quest select 1)], 
                        "Quest_Complete"]] call ML_fnc_showNotification;
  };
};
