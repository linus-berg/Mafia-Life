...
private ["_civ", "_handled", "_i", "_range", "_dirV", "_pos", "_posFind",
         "_men", "_atms", "_atm"];
...
for [{_i=1}, {_i < 3}, {_i=_i+1}] do {
  ...
  _posFind = [(_pos select 0)+(_dirV select 0)*_range,
              (_pos select 1)+(_dirV select 1)*_range,
              (_pos select 2)+(_dirV select 2)*_range];
