...
private["_emporium_list", "_config"];
_emporium_list = _this;
_config = [];
...
lbClear IDC_EMPORIUM_SELL_LIST;
{
	if((_x select 3) == 1)then{
		if((_x select 0) in _config)then{
			for "_i" from 1 to ([_config, (_x select 0)] call ML_fnc_Array_ElementCount) do {
				_index = lbAdd [IDC_EMPORIUM_SELL_LIST, ((_x select 0) call ML_fnc_Merchandise_GetName)];
				lbSetData [IDC_EMPORIUM_SELL_LIST, _index, (_x select 0)];
				lbSetValue [IDC_EMPORIUM_SELL_LIST, _index, ((_x select 1)/2)*1.25];
				lbSetPicture [IDC_EMPORIUM_SELL_LIST, _index, ((_x select 0) call ML_fnc_Merchandise_GetPicture)];
				lbSetTooltip [IDC_EMPORIUM_SELL_LIST, _index, format["Sell Price: %1€", ((_x select 1)/2)*1.25]];
			};
		};
	};
