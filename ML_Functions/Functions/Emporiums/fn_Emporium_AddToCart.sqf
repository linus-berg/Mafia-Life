...
_cur_cart = (count ML_EmporiumCart);

if(_item == "")exitWith{systemChat "No item selected!"; false};
if(_cur_cart >= 50)exitWith{systemChat "Your cart is full."; false};
if(_amount <= 0)exitWith{systemChat "Enter a positive amount"; false};
if((_cur_cart+_amount) >= 50)then{_amount = 50-_cur_cart;};
...
for "_i" from 1 to _amount do {
	ML_EmporiumCart pushBack _item;
	ML_EmporiumTotal = ML_EmporiumTotal + _price;

	_index = lbAdd [IDC_EMPORIUM_CART_LIST, (_item call ML_fnc_Merchandise_GetName)];
	lbSetData [IDC_EMPORIUM_CART_LIST, _index, _item];
	lbSetValue [IDC_EMPORIUM_CART_LIST, _index, _price];
	lbSetPicture [IDC_EMPORIUM_CART_LIST, _index, (_item call ML_fnc_Merchandise_GetPicture)];
