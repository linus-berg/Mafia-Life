...
if(!(player call ML_fnc_Client_IsArmed))exitWith{false};
if(!(_target call ML_fnc_Client_IsDefenceless))exitWith{systemChat format["%1 is not defenceless.", name _target]; false};
if(!([_target, "ML_LastStolen", 1200] call ML_fnc_Misc_CheckTimer))exitWith{systemChat format["%1 was recently stolen from.", name _target]; false};
